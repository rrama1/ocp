  ...
  return (
    <div>
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, var(--bg-secondary) 0%, rgba(234, 179, 8, 0.12) 100%)' }}>
        ...
        <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginTop: '0.2rem' }}>YAML Manifests & Templates Workbench</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
          {YAML_STRUCTURE_EXPLANATION.keys.map((item, idx) => {
            ...
            return (
              ...
              >
                <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '1.05rem', color: colors[idx % colors.length] }}>
                  {item.key}
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '1.5rem', marginTop: '2rem' }}>
        ...
        <div>
          ...
          <div className="card" style={{ padding: '1rem' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Layers size={14} /> Templates ({filteredTemplates.length})
            </div>
            <div style={{ display: 'grid', gap: '0.4rem', maxHeight: '420px', overflowY: 'auto' }}>
          ...
          <div className="card" style={{ marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
                ...
                <h3 style={{ fontSize: '1.4rem', fontWeight: 800, marginTop: '0.3rem' }}>{selectedTemplate.name}</h3>
                <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                  {selectedTemplate.description}
                </p>
              </div>

              <button className="btn btn-primary" onClick={handleCopy} style={{ fontSize: '0.85rem', gap: '0.4rem' }}>
                {copied ? <Check size={16} /> : <Copy size={16} />}
                {copied ? 'Copied to Clipboard!' : 'Copy YAML Manifest'}
              </button>
            </div>

            {/* Parameter Customizer Bar */}
            <div style={{ marginTop: '1.25rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.85rem' }}>
              <div>
                ...
                <input
                  ...
                  style={{ width: '100%', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', padding: '0.35rem 0.6rem', borderRadius: '4px', fontSize: '0.85rem' }}
                />
              </div>
